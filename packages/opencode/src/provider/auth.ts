...
export namespace ProviderAuth {
  ...
  export interface Interface {
    ...
    readonly callback: (input: { providerID: ProviderID; method: number; code?: string; authKey?: string }) => Effect.Effect<void, Error>
  }
  ...
  export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const pending = new Map<ProviderID, { result: AuthOuathResult; authKey?: string }>()
      ...
      const authorize = Effect.fn("ProviderAuth.authorize")(function* (input: {
        ...
        pending.set(input.providerID, { result, authKey: input.authKey })
        return {
          url: result.url,
      ...
      const callback = Effect.fn("ProviderAuth.callback")(function* (input: {
        ...
        const pendingAuth = pending.get(input.providerID)
        if (!pendingAuth) return yield* Effect.fail(new OauthMissing({ providerID: input.providerID }))
        const match = pendingAuth.result
        const target = input.authKey ?? pendingAuth.authKey ?? input.providerID
